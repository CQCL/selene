...
#[derive(Default)]
pub struct ExampleSimulatorFactory;
...
impl SimulatorInterfaceFactory for ExampleSimulatorFactory {
    ...
    fn init(
        ...
        match Params::try_parse_from(args) {
            Err(e) => Err(anyhow!("Error parsing arguments to example plugin: {}", e)),
            Ok(params) => Ok(Box::new(ExampleSimulator {
                n_qubits,
...
export_simulator_plugin!(crate::ExampleSimulatorFactory);
